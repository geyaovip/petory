...
const IDENTITY_RULES = `Use the main subject in the uploaded image as the only subject. This is a strict likeness task: someone who knows this subject must immediately recognize it in the output. Preserve the exact same individual — species or object type, silhouette, primary colors, material or fur texture, markings, face shape, eye color and shape, nose, ears, head details, body proportions, clothing, accessories, and every distinctive identifying detail. Do not redesign, simplify, beautify, genericize, replace, merge with a stock mascot, or drift toward any other character design.`

const POSE_SET_RULES = `This image is one pose in a fixed six-pose desktop-companion set. Keep the same rendering treatment, camera distance, camera height, three-quarter framing, upper-front-left lighting, material qualities, and apparent scale across the whole set. Only change the pose-specific body language and expression described below. Do not switch to a different art style, camera angle, crop, or character design.`

const RENDER_RULES = `Render it as a believable semi-realistic faux-3D desktop companion cutout while keeping the source subject's recognizable real-world appearance. Preserve the source subject's real anatomy, fur, feathers, skin, fabric, metal, or other material texture. Add only restrained three-dimensional volume, natural surface depth, soft contact shading, gentle ambient occlusion, subtle rim light, and clean product-photography lighting from the upper front-left. Use a medium focal length with minimal perspective distortion. The result should feel dimensional and alive while still looking like the uploaded subject, not a newly invented mascot. Do not make it flat, card-like, sticker-like, icon-like, cel-shaded, heavily illustrated, chibi, super-deformed, toy-like, plush-like, plastic, glossy figurine-like, or excessively cartoonish. Do not place it inside a card, badge, frame, tile, rounded rectangle, platform, room, or scenery.`
...
const SEATED_BASE = `Use the same seated upright three-quarter desktop-companion base: body turned slightly three-quarter while the face looks toward the viewer, hindquarters or equivalent base resting on the ground, both front paws planted naturally side by side unless one paw is intentionally raised, tail visible beside the body when the subject has a tail, and the full body readable in frame.`
...
const POSE_BODY: Record<PetPoseType, string> = {
  ...
  happy: `${SEATED_BASE} Keep the same seated upright body, camera framing, and scale as idle. Change only the expression and one paw gesture: use a gentle warm smile (subtle, not exaggerated) and raise one front paw in a small natural wave while the other paw and hind legs stay planted. Keep the mouth closed by default; only show teeth or an open mouth if the identity reference already shows that exact mouth shape. Do not change face shape, eyes, muzzle, markings, or any identity traits while smiling. The happiness must come from a small expression change and one lifted paw, not from standing, jumping, leaning far forward, or changing the camera angle.`,
